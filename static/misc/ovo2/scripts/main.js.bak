...
window.DOMElementHandler = class extends DOMHandler {
  constructor(a, b) {
    super(a, b),
      ...
      (this._autoAttach = !0),
      this.AddRuntimeMessageHandler("create", (a) => this._OnCreate(a)),
      this.AddRuntimeMessageHandler("destroy", (a) => this._OnDestroy(a)),
      this.AddRuntimeMessageHandler("set-visible", (a) =>
        this._OnSetVisible(a)
      ),
      this.AddRuntimeMessageHandler("update-position", (a) =>
        this._OnUpdatePosition(a)
      ),
      this.AddRuntimeMessageHandler("update-state", (a) =>
        this._OnUpdateState(a)
      ),
      this.AddRuntimeMessageHandler("focus", (a) => this._OnSetFocus(a)),
      this.AddRuntimeMessageHandler("set-css-style", (a) =>
        this._OnSetCssStyle(a)
      );
  }
  SetAutoAttach(a) {
    this._autoAttach = !!a;
  ...
  _OnDestroy(a) {
    ...
    this.DestroyElement(c),
      this._autoAttach && c.parentElement.removeChild(c),
  ...
  window.RuntimeInterface = class f {
    constructor(a) {
      (this._useWorker = a.useWorker),
        ...
        this.AddRuntimeComponentMessageHandler(
          ...
          (a) => this._OnCordovaFetchLocalFile(a)
        ),
        this.AddRuntimeComponentMessageHandler(
          "runtime",
          "create-job-worker",
          (a) => this._OnCreateJobWorker(a)
        ),
    ...
    async _Init(a) {
        ...
        for (let b = 0, c = a.engineScripts.length; b < c; ++b) {
          ...
          this._localFileStrings.hasOwnProperty(c)
            ...
            : this._localFileBlobs.hasOwnProperty(c) &&
              (a.engineScripts[b] = URL.createObjectURL(
                this._localFileBlobs[c]
              ));
        }
      }
      if (a.baseUrl) this._baseUrl = a.baseUrl;
        ...
        const b = this._baseUrl.lastIndexOf("/");
        -1 !== b && (this._baseUrl = this._baseUrl.substr(0, b + 1));
      }
      if (a.workerScripts)
        for (const [b, c] of Object.entries(a.workerScripts))
          this._workerScriptBlobURLs[b] = URL.createObjectURL(c);
      const b = new MessageChannel();
      (this._messageChannelPort = b.port1),
        (this._messageChannelPort.onmessage = (a) =>
          this["_OnMessageFromRuntime"](a.data)),
        window["c3_addPortMessageHandler"] &&
          window["c3_addPortMessageHandler"]((a) =>
            this._OnMessageFromDebugger(a)
          ),
        ...
        this._useWorker
          ? await this._InitWorker(a, b.port2)
          : await this._InitDOM(a, b.port2);
    }
    _GetWorkerURL(a) {
      return this._workerScriptBlobURLs.hasOwnProperty(a)
        ? this._workerScriptBlobURLs[a]
        ...
        ? URL.createObjectURL(this._localFileBlobs[a.toLowerCase()])
        : a;
    }
    async CreateWorker(a, b, c) {
      if (a.startsWith("blob:")) return new Worker(a, c);
      if (this.IsiOSCordova()) {
        const b = await this.CordovaFetchLocalFileAsArrayBuffer(
            this._scriptFolder + a
          ),
          d = new Blob([b], { type: "application/javascript" });
    ...
    MaybeForceBodySize() {
      if (this.IsiOSWebView()) {
        ...
        (b["height"] = a["height"] = e + "px"),
          (b["width"] = a["width"] = d + "px");
      }
    }
    ...
    async _InitWorker(a, b) {
      ...
      document.body.appendChild(this._canvas),
        ...
        this._worker.postMessage(
          ...
          [b, d, ...this._jobScheduler.GetPortTransferables()]
        ),
        (this._domHandlers = j.map((a) => new a(this))),
        this._FindRuntimeDOMHandler(),
        (self["c3_callFunction"] = (a, b) =>
          this._runtimeDomHandler._InvokeFunctionFromJS(a, b)),
    ...
    async _InitDOM(b, c) {
      (this._canvas = document.createElement("canvas")),
        ...
        (this._domHandlers = j.map((a) => new a(this))),
        this._FindRuntimeDOMHandler();
      const d = b.engineScripts.map((a) =>
        "string" == typeof a ? new URL(a, this._baseUrl).toString() : a
      );
      if (
        (Array.isArray(b.workerDependencyScripts) &&
          d.unshift(...b.workerDependencyScripts),
        await Promise.all(d.map((b) => a(b))),
        b.projectScripts && 0 < b.projectScripts.length)
      ) {
        const c = self["C3_ProjectScriptsStatus"];
        try {
          if (
            (await Promise.all(b.projectScripts.map((b) => a(b[1]))),
            Object.values(c).some((a) => !a))
          )
            return void self.setTimeout(
              () => this._ReportProjectScriptError(c),
              100
            );
        } catch (a) {
          return (
            console.error("[Preview] Error loading project scripts: ", a),
            void self.setTimeout(() => this._ReportProjectScriptError(c), 100)
          );
        }
      }
      if (
      ...
      (this._localRuntime = self["C3_CreateRuntime"](e)),
        await self["C3_InitRuntime"](this._localRuntime, e);
    }
    ...
    _OnEventFromRuntime(a) {
      ...
      if (!f)
        return void console.warn(
          `[DOM] No event handlers for component '${b}'`
        );
      const g = f.get(c);
      if (!g)
        return void console.warn(
          `[DOM] No handler '${c}' for component '${b}'`
        );
      let h = null;
      try {
        h = g(d);
        ...
        return (
          console.error(`Exception in '${b}' handler '${c}':`, a),
          void (null !== e && this._PostResultToRuntime(e, !1, "" + a))
        );
      }
      null !== e &&
        (h && h.then
          ? h
              .then((a) => this._PostResultToRuntime(e, !0, a))
              .catch((a) => {
                console.error(`Rejection from '${b}' handler '${c}':`, a),
                  this._PostResultToRuntime(e, !1, "" + a);
              })
          : this._PostResultToRuntime(e, !0, h));
    }
    _PostResultToRuntime(a, b, c) {
      let d;
      c && c["transferables"] && (d = c["transferables"]),
        this._messageChannelPort.postMessage(
          { type: "result", responseId: a, isOk: b, result: c },
          d
        );
    }
    ...
    AddRuntimeComponentMessageHandler(a, b, c) {
      let d = k.get(a);
      if ((d || ((d = new Map()), k.set(a, d)), d.has(b)))
        throw new Error(`[DOM] Component '${a}' already has handler '${b}'`);
      d.set(b, c);
    }
    ...
    _FindRuntimeDOMHandler() {
      for (const a of this._domHandlers)
        if ("runtime" === a.GetComponentID())
          return void (this._runtimeDomHandler = a);
    ...
    async _WasmDecodeWebMOpus(a) {
      const b = await this.PostToRuntimeComponentAsync(
        "runtime",
        "opus-decode",
        { arrayBuffer: a },
        null,
    ...
    async _ConvertDataUrisToBlobs() {
      const a = [];
      for (const [b, c] of Object.entries(this._localFileBlobs))
        a.push(this._ConvertDataUriToBlobs(b, c));
      await Promise.all(a);
    }
    async _ConvertDataUriToBlobs(a, b) {
      if ("object" == typeof b)
        (this._localFileBlobs[a] = new Blob([b["str"]], { type: b["type"] })),
          (this._localFileStrings[a] = b["str"]);
      else {
        let c = await this._FetchDataUri(b);
        c || (c = this._DataURIToBinaryBlobSync(b)),
          (this._localFileBlobs[a] = c);
      }
    }
    ...
    _ParseDataURI(a) {
      ...
      return (
        (i =
          "base64" === g || "base64" === h ? atob(d) : decodeURIComponent(d)),
        { mime_type: f, data: i }
      );
    }
    ...
    _TestTransferablesWork() {
      ...
      return (
        ...
        d.port1.postMessage({ arrayBuffer: c }, [c]),
        b
      );
    }
  };
...
("use strict");
{
  ...
  async function f(a, b, c) {
    ...
    return (
      (f = j.serializeToString(h)),
      (a = new Blob([f], { type: "image/svg+xml" })),
      await d(a)
    );
  }
  function g(a) {
  ...
  function k() {
    ...
    return (
      !("textarea" !== b) ||
      ("input" === b ? c.has(a.type.toLowerCase() || "text") : g(a))
    );
  }
  const l = new Map([
      ["OSLeft", "MetaLeft"],
    ...
    m = { dispatchRuntimeEvent: !0, dispatchUserScriptEvent: !0 },
  ...
  let q = !1;
  document.addEventListener("pause", () => (q = !0)),
    document.addEventListener("resume", () => (q = !1));
  const r = class extends DOMHandler {
    constructor(a) {
      super(a, "runtime"),
        ...
        a.AddRuntimeComponentMessageHandler("canvas", "update-size", (a) =>
          this._OnUpdateCanvasSize(a)
        ),
        a.AddRuntimeComponentMessageHandler("runtime", "invoke-download", (a) =>
          this._OnInvokeDownload(a)
        ),
        a.AddRuntimeComponentMessageHandler(
          "runtime",
          "raster-svg-image",
          (a) => this._OnRasterSvgImage(a)
        ),
        a.AddRuntimeComponentMessageHandler(
          "runtime",
          "get-svg-image-size",
          (a) => this._OnGetSvgImageSize(a)
        ),
        a.AddRuntimeComponentMessageHandler(
          "runtime",
          "set-target-orientation",
          (a) => this._OnSetTargetOrientation(a)
        ),
        ...
        a.AddRuntimeComponentMessageHandler(
          "runtime",
          "post-to-debugger",
          (a) => this._OnPostToDebugger(a)
        ),
        a.AddRuntimeComponentMessageHandler("runtime", "go-to-script", (a) =>
          this._OnPostToDebugger(a)
        ),
        a.AddRuntimeComponentMessageHandler(
          "runtime",
        ...
        a.AddRuntimeComponentMessageHandler("runtime", "add-stylesheet", (a) =>
          this._OnAddStylesheet(a)
        ),
        a.AddRuntimeComponentMessageHandler("runtime", "alert", (a) =>
          this._OnAlert(a)
        );
      ...
      window.addEventListener("selectstart", h),
        ...
        c.addEventListener("selectstart", h),
        c.addEventListener("gesturehold", h),
        ...
        window.addEventListener("mousewheel", i, { passive: !1 }),
    ...
    Attach() {
      window.addEventListener("focus", () =>
        ...
        window.addEventListener("blur", () => {
          this._PostRuntimeEvent("window-blur", { parentHasFocus: j() }),
            (this._mousePointerLastButtons = 0);
        }),
        ...
        window.addEventListener("fullscreenerror", (a) =>
          this._OnFullscreenError(a)
        ),
        window.addEventListener("webkitfullscreenerror", (a) =>
          this._OnFullscreenError(a)
        ),
        window.addEventListener("mozfullscreenerror", (a) =>
          this._OnFullscreenError(a)
        ),
        ...
        window.addEventListener("dblclick", (a) =>
          this._OnMouseEvent("dblclick", a, m)
        ),
    ...
    _OnFullscreenError(a) {
      console.warn("[Construct 3] Fullscreen request failed: ", a),
    ...
    _OnMouseEvent(b, c, d) {
      a(c) ||
        ("mousedown" === b && window !== window.top && window.focus(),
        this._PostToRuntimeMaybeSync(
          b,
          {
            button: c.button,
    ...
    _OnMouseEventAsPointer(b, c) {
      if (a(c)) return;
      "pointerdown" === b && window !== window.top && window.focus();
      const d = this._mousePointerLastButtons;
      "pointerdown" === b && 0 !== d
        ? (b = "pointermove")
        : "pointerup" == b && 0 !== c.buttons && (b = "pointermove"),
        this._PostToRuntimeMaybeSync(
          b,
          {
            pointerId: 1,
    ...
    _OnPointerEvent(a, b) {
      ...
      if (
        ...
        "pointerdown" === a
          ? (c = "mousedown")
          : "pointerup" == a && (c = "pointerup"),
          this._OnMouseEvent(c, b, n),
    ...
    _OnUpdateCanvasSize(a) {
      const b = this.GetRuntimeInterface(),
        c = b.GetCanvas();
      (c.style.width = a["styleWidth"] + "px"),
        (c.style.height = a["styleHeight"] + "px"),
        (c.style.marginLeft = a["marginLeft"] + "px"),
        (c.style.marginTop = a["marginTop"] + "px"),
        b.MaybeForceBodySize(),
        this._isFirstSizeUpdate &&
          ((c.style.display = ""), (this._isFirstSizeUpdate = !1));
    }
    _OnInvokeDownload(b) {
      const c = b["url"],
        d = b["filename"],
    ...
    async _OnRasterSvgImage(a) {
      ...
      let i;
      return (
        (i = g ? await createImageBitmap(h, g) : await createImageBitmap(h)),
        { imageBitmap: i, transferables: [i] }
      );
    }
    ...
    _OnDebugHighlight(a) {
      ...
      (c.style.display = ""),
        (c.style.left = a["left"] - 1 + "px"),
        (c.style.top = a["top"] - 1 + "px"),
        (c.style.width = a["width"] + 2 + "px"),
        (c.style.height = a["height"] + 2 + "px"),
    ...
    _OnAlert(a) {
      alert(a["message"] + " [via Web Worker]");
    }
  };
...
("use strict");
{
  ...
  self.JobSchedulerDOM = class {
    ...
    async _CreateJobWorker() {
      ...
      return (
        this._dispatchWorker.postMessage(
          { type: "_addJobWorker", port: d.port1 },
          [d.port1]
        ),
        c.postMessage(
          {
...
("use strict");
{
  const a = class extends DOMHandler {
    constructor(a) {
      super(a, "browser"),
        (this._exportType = ""),
        this.AddRuntimeMessageHandler("get-initial-state", (a) =>
          this._OnGetInitialState(a)
        ),
        this.AddRuntimeMessageHandler("ready-for-sw-messages", () =>
          this._OnReadyForSWMessages()
        ),
        this.AddRuntimeMessageHandler("alert", (a) => this._OnAlert(a)),
        this.AddRuntimeMessageHandler("close", () => this._OnClose()),
        this.AddRuntimeMessageHandler("set-focus", (a) => this._OnSetFocus(a)),
        this.AddRuntimeMessageHandler("vibrate", (a) => this._OnVibrate(a)),
        this.AddRuntimeMessageHandler("lock-orientation", (a) =>
          this._OnLockOrientation(a)
        ),
        this.AddRuntimeMessageHandler("unlock-orientation", () =>
          this._OnUnlockOrientation()
        ),
        this.AddRuntimeMessageHandler("navigate", (a) => this._OnNavigate(a)),
        this.AddRuntimeMessageHandler("request-fullscreen", (a) =>
          this._OnRequestFullscreen(a)
        ),
        ...
        "undefined" != typeof Windows &&
          Windows["UI"]["Core"]["SystemNavigationManager"]
            ["getForCurrentView"]()
            .addEventListener("backrequested", (a) =>
              this._OnWin10BackRequested(a)
            );
    }
    ...
    _OnRequestFullscreen(a) {
      ...
      1 === c
        ? (b["navigationUI"] = "hide")
        : 2 === c && (b["navigationUI"] = "show");
      const d = document.documentElement;
      d["requestFullscreen"]
        ? d["requestFullscreen"](b)
...
("use strict");
{
  function a(c, a) {
    return (
      !(c.length !== a.length) &&
      (!(c !== a) || c.toLowerCase() === a.toLowerCase())
    );
  }
  const b = class extends DOMHandler {
    constructor(a) {
      super(a, "audio"),
        ...
        (self["C3Audio_OnMicrophoneStream"] = (a, b) =>
          this._OnMicrophoneStream(a, b)),
    ...
    async _CreateAudioContext(a) {
      ...
      const b = { latencyHint: a["latencyHint"] };
      ...
      try {
        await Promise.all(
          a["preloadList"].map((a) =>
            this._GetAudioBuffer(a["originalUrl"], a["url"], a["type"], !1)
          )
        );
      } catch (a) {
    ...
    _ReconnectEffects(a) {
      ...
      if (c && c.length) {
        ...
        for (let a = 0, b = c.length; a < b; ++a) {
          const d = c[a];
          a + 1 === b
            ? d.ConnectTo(this.GetDestinationNode())
            : d.ConnectTo(c[a + 1].GetInputNode());
        }
      }
    ...
    DecodeAudioData(a, b) {
      return b
        ? this._iRuntime._WasmDecodeWebMOpus(a).then((a) => {
    ...
    *audioInstancesByTag(b) {
    ...
    _AddPendingTag(a) {
      ...
      if (!b) {
        ...
        (b = { pendingCount: 0, promise: d, resolve: c }),
    ...
    async _Play(a) {
      ...
      if (0 < k && !a["trueClock"])
        if (this._audioContext["getOutputTimestamp"]) {
          ...
          k = k - performance.now() / 1e3 + this._audioContext["currentTime"];
        ...
        (this._lastAudioInstance = await this._GetAudioInstance(b, c, d, f, e)),
          j
            ? (this._lastAudioInstance.SetPannerEnabled(!0),
              ...
              j.hasOwnProperty("uid") &&
                this._lastAudioInstance.SetUID(j["uid"]))
            : this._lastAudioInstance.SetPannerEnabled(!1),
          this._lastAudioInstance.Play(g, h, i, k);
      } catch (a) {
    ...
    async _Unload(a) {
      ...
      if (e) {
        e.Release();
        const a = this._audioBuffers.indexOf(e);
    ...
    _OnTick(a) {
      ...
      for (const b of a["instPans"]) {
        const a = b["uid"];
        for (const c of this._audioInstances)
          c.GetUID() === a && c.SetPanXYA(b["x"], b["y"], b["angle"]);
      }
    }
    async _AddEffect(a) {
      const b = a["type"],
      ...
      if ("filter" === b) e = new C3AudioFilterFX(this, ...d);
        ...
        (e = new C3AudioConvolveFX(this, b.GetAudioBuffer(), ...d)),
          e._SetBufferInfo(
            a["bufferOriginalUrl"],
            a["bufferUrl"],
            a["bufferType"]
    ...
    _SetEffectParam(a) {
      ...
      !h ||
        0 > c ||
        c >= h.length ||
        (h[c].SetParam(d, e, f, g), this._PostUpdatedFxState());
    }
    _RemoveEffects(a) {
    ...
    async _OnLoadState(a) {
      ...
      if (3 !== b)
        for (const a of this._audioInstances)
          (a.IsMusic() && 1 === b) || (!a.IsMusic() && 2 === b) || a.Stop();
      for (const b of this._effects.values()) for (const a of b) a.Release();
      this._effects.clear(),
        (this._timeScale = a["timeScale"]),
        (this._gameTime = a["gameTime"]);
      const c = a["listenerPos"];
      ...
      const d = [];
      for (const b of Object.values(a["effects"]))
        d.push(Promise.all(b.map((a) => this._AddEffect(a))));
      await Promise.all(d),
        await Promise.all(
          a["playing"].map((a) => this._LoadAudioInstance(a, b))
        ),
        this._MaybeStartTicking();
    }
    async _LoadAudioInstance(a, b) {
      ...
      k.LoadPanState(a["pan"]),
        k.Play(h, i, j, 0),
        a["isPlaying"] || k.Pause(),
        k._LoadAdditionalState(a);
    }
...
self.C3AudioBuffer = class {
  ...
  static Create(a, b, c, d, e) {
    ...
    return (
      ...
      !e || a.IsPlayMusicAsSound() || f
        ? new C3WebAudioBuffer(a, b, c, d, e, f)
        : new C3Html5AudioBuffer(a, b, c, d, e)
    );
  }
  CreateInstance(a) {
    return "html5" === this._api
      ? new C3Html5AudioInstance(this._audioDomHandler, this, a)
      : new C3WebAudioInstance(this._audioDomHandler, this, a);
  }
  _Load() {}
  Load() {
    return (
...
self.C3Html5AudioBuffer = class extends C3AudioBuffer {
  ...
  _OnError(a) {
    console.error(`[Construct 3] Audio '${this._url}' error: `, a),
      this._loadReject &&
  ...
  IsLoaded() {
    ...
    return (
      a && (this._reachedCanPlayThrough = !0), a || this._reachedCanPlayThrough
    );
  }
  IsLoadedAndDecoded() {
...
self.C3WebAudioBuffer = class extends C3AudioBuffer {
  ...
  async _Fetch() {
    ...
    if ("cordova" === a.GetExportType() && a.IsRelativeURL(this._url))
  ...
  async _Decode() {
    return this._audioBuffer
      ? this._audioBuffer
      : void ((this._audioBuffer = await this._audioDomHandler.DecodeAudioData(
          this._audioData,
  ...
  async _Load() {
    try {
      ...
      (this._loadState = "failed"),
        console.error(`[Construct 3] Failed to load audio '${this._url}': `, a);
    }
  }
  IsLoaded() {
...
("use strict");
{
  ...
  self.C3AudioInstance = class {
    constructor(a, b, d) {
      ...
      (this._isTimescaled = (1 === e && !this.IsMusic()) || 2 === e),
        (this._instUid = -1),
        (this._fadeEndTime = -1),
    ...
    GetDuration(a) {
      ...
      return a && (b /= this._playbackRate || 0.001), b;
    }
    Play() {}
    Stop() {}
    Pause() {}
    ...
    SetPanXYA(a, b, c) {
      this._isPannerEnabled &&
        (this._pannerNode["setPosition"](a, b, 0),
        this._pannerNode["setOrientation"](Math.cos(c), Math.sin(c), 0));
    }
    SetUID(a) {
      this._instUid = a;
    }
    ...
    GetPanState() {
      ...
      return {
        ...
        orient: [
          a["orientationX"]["value"],
          a["orientationY"]["value"],
          a["orientationZ"]["value"],
        ],
    ...
    LoadPanState(a) {
      ...
      b["setPosition"](...b["pos"]),
        ...
        (b["coneInnerAngle"] = b["cia"]),
        (b["coneOuterAngle"] = b["coa"]),
        (b["coneOuterGain"] = b["cog"]),
        (this._instUid = b["uid"]);
    }
  };
}
...
self.C3Html5AudioInstance = class extends C3AudioInstance {
  ...
  GetPlaybackTime(a) {
    ...
    return (
      a && (b *= this._playbackRate),
      this._isLooping || (b = Math.min(b, this.GetDuration())),
      b
    );
  }
  Play(a, b, c) {
    ...
    if (
      ...
      try {
        ...
        console.warn(
          `[Construct 3] Exception seeking audio '${this._buffer.GetUrl()}' to position '${c}': `,
          a
        );
      }
...
self.C3WebAudioInstance = class extends C3AudioInstance {
  constructor(a, b, c) {
    super(a, b, c),
      ...
      (this._onended_handler = (a) => this._OnEnded(a)),
  ...
  Play(a, b, c, d) {
    (this._muteVol = 1),
      ...
      (this._isLooping = a),
      (this._playbackRate = 1),
      (this._startTime = this.GetCurrentTime() - c);
  }
  Stop() {
...
("use strict");
{
  ...
  class f {
    ...
    SetAudioParam(a, b, c, d) {
      if ((a["cancelScheduledValues"](0), 0 === d))
        return void (a["value"] = b);
  ...
  (self.C3AudioFilterFX = class extends f {
      ...
      SetParam(a, c, d, e) {
        0 === a
          ? ((c = Math.max(Math.min(c / 100, 1), 0)),
            (this._params[2] = c),
            this.SetAudioParam(this._wetNode["gain"], c, d, e),
            this.SetAudioParam(this._dryNode["gain"], 1 - c, d, e))
          : 4 === a
          ? ((this._params[1] = b(c)),
            this.SetAudioParam(this._delayGainNode["gain"], b(c), d, e))
          : 5 === a
          ? ((this._params[0] = c),
            this.SetAudioParam(this._delayNode["delayTime"], c, d, e))
          : void 0;
      }
    }),
    ...
    (self.C3AudioGainFX = class extends f {
      ...
      SetParam(a, c, d, e) {
        4 === a
          ? ((this._params[0] = b(c)),
            this.SetAudioParam(this._node["gain"], b(c), d, e))
          : void 0;
      }
    }),
    (self.C3AudioTremoloFX = class extends f {
      ...
      SetParam(a, b, c, d) {
        0 === a
          ? ((b = Math.max(Math.min(b / 100, 1), 0)),
            (this._params[1] = b),
            this.SetAudioParam(this._node["gain"]["value"], 1 - b / 2, c, d),
    ...
    (self.C3AudioDistortionFX = class extends f {
      ...
      _GenerateColortouchCurve(a, b) {
        for (let c, d = 0; d < 32768; ++d)
          (c = d / 32768),
            (c = this._Shape(c, a, b)),
            (this._curve[32768 + d] = c),
            (this._curve[32768 - d - 1] = -c);
      }
      _Shape(a, b, c) {
        const d = 1.05 * c * b - b,
          f = 0 > a ? -1 : 1,
    ...
    (self.C3AudioAnalyserFX = class extends f {
      ...
      Tick() {
        ...
        let b = 0;
        for (let c, d = 0; d < a; ++d)
          (c = (this._signal[d] - 128) / 128),
            0 > c && (c = -c),
            this._peak < c && (this._peak = c),
            (b += c * c);
...
("use strict");
{
      ...
      return (
        "textarea" === f
          ? ((e = document.createElement("textarea")),
            (e.style.resize = "none"))
          : ((e = document.createElement("input")), (e.type = f)),
        ...
        e.addEventListener("click", (a) => {
          a.stopPropagation(), this._PostToRuntimeElementMaybeSync("click", c);
        }),
        e.addEventListener("dblclick", (a) => {
          a.stopPropagation(),
            this._PostToRuntimeElementMaybeSync("dblclick", c);
        }),
        e.addEventListener("input", () =>
          this.PostToRuntimeElement("change", c, { text: e.value })
        ),
        (e.id = d["id"]),
        this.UpdateState(e, d),
        e
      );
    }
    UpdateState(a, b) {
  ...
  RuntimeInterface.AddDOMHandlerClass(c);
}
